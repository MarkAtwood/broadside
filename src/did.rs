...
/// Dots in domain are preserved per the did:web spec.
...
pub fn private_key_to_mnemonic(private_key: &[u8; 32]) -> String {
    let mnemonic =
        bip39::Mnemonic::from_entropy(private_key).expect("32 bytes is valid BIP-39 entropy");
    mnemonic.to_string()
}
...
/// verification method when present.
pub fn did_web_document(
    ...
    let mut verification_methods = vec![serde_json::json!({
    ...
    let mut contexts: Vec<serde_json::Value> =
        vec![serde_json::json!("https://www.w3.org/ns/did/v1")];
...
pub fn hex_decode(s: &str) -> anyhow::Result<Vec<u8>> {
    anyhow::ensure!(s.len() % 2 == 0, "invalid hex string length");
    (0..s.len())
        .step_by(2)
        .map(|i| {
...
mod tests {
    ...
    fn mnemonic_round_trip() {
        ...
        assert_eq!(
            recovered_pub, public_key,
            "Recovered key must derive same pubkey"
        );
    }
    ...
    fn did_web_document_structure() {
        ...
        assert_eq!(
            methods.len(),
            2,
            "Should have RSA + Ed25519 verification methods"
        );
    ...
    fn did_web_document_empty_aka() {
        ...
        assert!(
            ...
            "Empty alsoKnownAs should be null"
        );
    }
    ...
    fn did_web_document_no_recovery_key() {
        ...
        assert_eq!(
            methods.len(),
            1,
            "Should only have RSA key when no recovery key"
        );
        assert_eq!(methods[0]["type"], "RsaVerificationKey2018");
        ...
        assert_eq!(
            contexts.len(),
            1,
            "Should only have DID context when no Ed25519 key"
        );
    }
